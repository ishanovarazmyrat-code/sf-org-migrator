...
const DETERMINISTIC_CODES = new Set([
  ...
  // Since the duplicate-rule header went in, an "Allow" rule no longer blocks
  // us — so a DUPLICATES_DETECTED that still gets through is a rule whose
  // action is "Block", which rejects the same record every time.
  'DUPLICATES_DETECTED',
